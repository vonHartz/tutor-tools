...
temp_dir = 'temp917'
dont_check = ['.svn', 'private', 'public', '_Feedback', 'internal', 'sample_solutions', temp_dir]
...
def main(sheet_no, rz_short, submissions_dir, credits_dir, json_prefix, parse_credits_script):
    """
...
def gather_feedback(sheet_no):
    ...
    paths = [p for p in os.listdir() if os.path.isdir(p) and p not in dont_check]
    for path in paths:
